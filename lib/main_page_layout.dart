    ...
    return Row(
      children: <Widget>[
        ...
        Expanded(
          child: TextButton(
            onPressed: () async {
              final weatherInfo = await fetchYumemiWeather();

              // APIの取得に成功した場合
              if (weatherInfo is Success<WeatherInfoModel>) {
                _updateWeatherCondition(
                  weatherInfo.value.weatherCondition,
                );
              }

              // APIの取得に失敗した場合
              if (weatherInfo is Failure<WeatherInfoModel>) {
